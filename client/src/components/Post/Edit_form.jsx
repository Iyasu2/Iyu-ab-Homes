...
const PropertyForm = ({ property }) => {
  ...
  const [formData, setFormData] = useState({
    type: property.type || "",
    totalArea: (property.totalArea && property.totalArea.split(" ")[0]) || "",
    builtInArea:
      (property.builtInArea && property.builtInArea.split(" ")[0]) || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("property: ", formattedFormData);
      ...
      if (!response.ok) {
        ...
        setError(null);
        setEmptyFields([]);
        dispatch({ type: "UPDATE_PROPERTY", payload: responseData });
      }
    } catch (error) {
