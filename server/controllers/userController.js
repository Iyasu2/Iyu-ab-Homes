...
const getUserData = async (req, res) => {
  ...
  try {
    ...
    const { fullName, telegram, whatsapp, facebook, phoneNumber, profileImage } = user;
    const fullProfileImagePath = profileImage ? `http://localhost:5000/${profileImage}` : null;
    res.status(200).json({ fullName, telegram, whatsapp, facebook, phoneNumber, profileImage: fullProfileImagePath });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
